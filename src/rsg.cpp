...
List rsg (List adj_list,
  ...
  while(Ndist_current > Ndistrict){
    ...
    Rcout << "Start Step 2b" << std::endl;
    j_dist_vec = runif(1, 0, 1000000000);
    Rcout << "Size of idist_dneighbors is " << idist_dneighbors.size()
	  << std::endl;
    Rcout << "Entry in j_dist_vec(0) is " << j_dist_vec(0) << std::endl; 
    j_dist = idist_dneighbors[fmod(j_dist_vec(0), idist_dneighbors.size())];
    Rcout << "j_dist is " << j_dist << std::endl;
    jdist_pmembers = IntegerVector(member_plist[j_dist]);
    Rcout << "end step 2b" << std::endl << std::endl;
